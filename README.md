...
6. Автозавершение обучения если тренировка начала выдавать худшие резултаты.
7. Лучшая модель сохраняется только в конце обучения, а не перезаписывается каждую эпоху. А чекпоинты каждые 10 эпох. Это экономит время и не изнашивает жесткий диск.
8. Создание датасета по субтитрам. (см. в finetune_models https://github.com/Le107/xtts-finetune-webui-mod/blob/main/finetune_models/readme.md). Когда нарезается датасет по субтитрам создается файл `Dialogs.txt`, в котором сохраняется `имя_аудиофайла_датасета`=`текст_из_субтитров`.
9. Добавлен скрипт перевода `Dialogs.txt` на другой язык - `translate_dialogs.py` (см. в finetune_models). Сохраняет перевод отдельным файлом `Dialogs_*.txt`, где * - язык, на который переводит(en, ru,... или другой).
10. Добавлен пакетный способ генерации, где вместо reference.wav(эталоный звук динамика) файл берет из датасета, а текст из `Dialogs.txt`(или `Dialogs_*.txt`, по выбору), и сохраняет в папку `translated_output`.
