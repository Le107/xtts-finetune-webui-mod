...
class Trainer:
    def __init__(  # pylint: disable=dangerous-default-value
        ...
        self.last_step_checkpoint_path = None
...
#                    self.save_checkpoint()
        ...
        # --- Оптимизированное сохранение каждые 250 шагов с удалением старых ---
        if self.total_steps_done % 250 == 0:
            print(f" --- Сохранение чекпоинта (Шаг {self.total_steps_done}) ---")
            model_state = self.model.module.state_dict() if hasattr(self.model, "module") else self.model.state_dict()
            optimized_model_state = {k: v for k, v in model_state.items() if "dvae" not in k}
                                
            checkpoint_name = f"checkpoint_step_{self.total_steps_done}.pth"
            current_checkpoint_path = os.path.join(self.output_path, checkpoint_name)
            
            checkpoint_data = {
                "config": self.config.to_dict(),
                "model": optimized_model_state,
                "step": self.total_steps_done,
                "epoch": self.epochs_done,
            }
            
            torch.save(checkpoint_data, current_checkpoint_path)
            print(f"\n > [STEP SAVE] Оптимизированный чекпоинт сохранен: {current_checkpoint_path}")

            # Удаление предыдущего шагового чекпоинта
            if self.last_step_checkpoint_path and os.path.exists(self.last_step_checkpoint_path):
                try:
                    os.remove(self.last_step_checkpoint_path)
                    print(f" > [STEP SAVE] Удален предыдущий чекпоинт: {self.last_step_checkpoint_path}")
                except OSError as e:
                    print(f" ! [STEP SAVE] Ошибка при удалении: {e}")

            # Обновляем путь к последнему сохраненному файлу
            self.last_step_checkpoint_path = current_checkpoint_path
        # -----------------------------------------------------------------------
    ...
    def _fit(self) -> None:
        ...
        for epoch in range(self.config.epochs):
            ...
            if getattr(self, "is_auto_stopping", False):
                break
            self.callbacks.on_epoch_end(self)
            self.start_with_eval = False
            
        return getattr(self, "best_eval_loss_ever", 999.0) 
    ...
    def save_best_model(self) -> None:
        ...
        if not hasattr(self, "best_eval_loss_ever"):
            self.best_eval_loss_ever = float("inf")
            self.is_auto_stopping = False

        if eval_loss > self.best_eval_loss_ever:
            print(f" [!] Внимание: Loss вырос ({eval_loss:.5f} > {self.best_eval_loss_ever:.5f})")
            print(" Модель начала переобучаться. Сохраняю финал. Заканчиваю обучение...")
            self.is_auto_stopping = True
        else:
            self.best_eval_loss_ever = eval_loss
            print(f" [+] Новый рекорд: {eval_loss:.5f}. Продолжаем обучение...")

        # Подготовка оптимизированного state_dict (общий для чекпоинтов и модели)
        model_state = self.model.module.state_dict() if hasattr(self.model, "module") else self.model.state_dict()
        optimized_model_state = {k: v for k, v in model_state.items() if "dvae" not in k}
        
        # Инициализируем атрибут для хранения пути, если его еще нет
        if not hasattr(self, "last_checkpoint_path"):
            self.last_checkpoint_path = None

        if (self.epochs_done + 1) % 10 == 0:
            print(f" --- Сохранение чекпоинта (Эпоха {self.epochs_done + 1}) ---")
            
            # Сначала готовим новый путь и сохраняем
            ...
            torch.save(checkpoint_data, current_checkpoint_path)
            print(f" > Оптимизированный чекпоинт сохранен: {current_checkpoint_path}")

            # Только теперь удаляем предыдущий чекпоинт
            if self.last_checkpoint_path and os.path.exists(self.last_checkpoint_path):
                try:
                    os.remove(self.last_checkpoint_path)
                    print(f" > Удален предыдущий чекпоинт: {self.last_checkpoint_path}")
                except OSError as e:
                    print(f" [!] Ошибка при удалении старого чекпоинта: {e}")

            # Обновляем ссылку на последний актуальный файл
            self.last_checkpoint_path = current_checkpoint_path

        is_last_epoch = (self.epochs_done + 1 >= self.config.epochs)
        
        if not (is_last_epoch or self.is_auto_stopping):
            return 

        print(" [□] Финализация: Сохранение оптимизированной модели...")
        ...
        final_model_path = os.path.join(self.output_path, "model.pth")
        torch.save(final_state, final_model_path)
        print(f" [☼] Оптимизированная модель готова в папке обучения: {final_model_path}")
        
        loss_file = os.path.join(self.output_path, "last_avg_loss.txt")
        try:
            with open(loss_file, "w") as f:
                f.write(str(eval_loss))
            print(f" [▼] Значение Loss ({eval_loss:.5f}) сохранено в: {loss_file}")
        except Exception as e:
            print(f" [!] Ошибка записи Loss в файл: {e}")
            
        if self.is_auto_stopping:
            return
