...
def train(model, optimizer, scheduler, criterion, data_loader, eval_data_loader, global_step):
    ...
    for epoch in range(c.epochs):
        ...
        if c.run_eval:
            ...
            if (epoch + 1) == c.epochs:
                print(f" --- ФИНАЛЬНОЕ СОХРАНЕНИЕ ЛУЧШЕЙ МОДЕЛИ (Эпоха {epoch + 1}) ---")
                ...
                # В остальных случаях просто обновляем цифры в памяти, не трогая диск
                if best_loss["eval_loss"] is None or eval_loss < best_loss["eval_loss"]:
                    best_loss["eval_loss"] = eval_loss
            model.train()
