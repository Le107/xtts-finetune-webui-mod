...
`Sub_to_dataset.py` нарежет звуковые файлы из папки `work` по субтитрам, также создаст файлы `metadata_eval.csv`(15% от датасета, минимум 1 запись) `metadata_train.csv`(85% от датасета) и поместит все в `
